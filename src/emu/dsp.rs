...
pub fn write_u16(ctx: &mut Context, register: u32, val: u16) {
    match register {
        MAILBOX_CPU_HI => ctx.dsp.mailbox_cpu = ctx.dsp.mailbox_cpu.set_hi(0x8000),
        MAILBOX_CPU_LO => {
        ...
        AR_DMA_MMAADDR_HI => ctx.dsp.ar_mma_addr_high = val,
        AR_DMA_MMAADDR_LO => ctx.dsp.ar_mma_addr_low = val,
        AR_DMA_ARADDR_HI => ctx.dsp.ar_ar_addr_high = val,
        AR_DMA_ARADDR_LO => ctx.dsp.ar_ar_addr_low = val,
        AR_DMA_SIZE_HI => ctx.dsp.ar_dma_size_high = val,
        AR_DMA_SIZE_LO => {
